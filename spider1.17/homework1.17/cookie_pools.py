...
class CookiesPool:
    ...
    def gen_cookies(self, loginname, password):
        if loginname not in self.id_list:
            try:
                self.driver.delete_all_cookies()  # 删除浏览器原有的Cookie
    ...
    def save(self, username, password, cookies, home_url):
        ...
        if username not in self.id_list:
            try:
                ...
                strsql='insert into cookies_0 VALUES(0,%s,%s,%s,%s)'
    ...
    def get_proxy(self):
        ...
        strsql='select * from cookies_0 where username="'+username +'"'
        self.cur.execute(strsql)
    ...
    def delCookies(self, username):
        ...
        :return:
        '''
        strsql = 'delete from cookies_0 where username="' + username + '"'
        self.cur.execute(strsql)
        self.conn.commit()
...
if __name__ == '__main__':
    pool = CookiesPool()
    account = [["17625809083","liye361365"],['3414018462@qq.com', 'qikuedu9527']]
    for i in account:
        print(i)
        pool = CookiesPool()
        result = pool.gen_cookies(i[0],i[1])
        print('result:',result)
        if result != None:
            cookies = result[0]
            home_url = result[1]
            pool.save(i[0],i[1],cookies,home_url)
